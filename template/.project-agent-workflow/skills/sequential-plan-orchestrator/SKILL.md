...
4. Admit one admissible implementation slice before running `.project-agent-workflow/scripts/run-sandboxed-plan-worker.py run <plan>`. It must establish one invariant, be independently reviewable and validatable, and retain an explicit later integration gate; a broad plan is not itself a valid candidate boundary. Delegation must produce independently useful output whose expected context reduction, parallelism, or review value exceeds coordination cost.
   ...
   For high-risk or correction-capable work, initialize the parent-owned `.project-agent-workflow/scripts/plan-execution-state.py` ledger outside the repository and pass it as `--plan-execution-state` to every runner operation. Record delegated and parent-direct review events before continuing. A hard trigger changes the ledger to `replan_required`; the runner gate then rejects before starting worker prerequisites.
