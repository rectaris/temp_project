...
Read `docs/agent/external-services.yaml` before accessing a provider. A missing `browser_run` record is disabled: do not infer authorization from a configured tool, and use the configured fallback.
