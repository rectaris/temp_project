...
2. Run `copier update --trust` without force-overwriting local conflicts.
3. Confirm that project-owned files are unchanged except for `model` and `model_reasoning_effort` in seeded `.codex/agents/*.toml` files.
4. Confirm the command exits nonzero when unresolved index conflicts, `*.rej` files, complete inline conflict blocks, or unclassified tracked-file deletion remain.
5. Search for `*.rej` files and complete inline conflict blocks.
6. Run `.project-agent-workflow/scripts/migrate-legacy-template-files.py` only when migrating a project generated by the pre-namespace layout.
7. Run `.project-agent-workflow/scripts/validate-changes.py --all` and the repository-specific validation suite.
8. Review and commit the update as one scoped change.

This template always runs a post-render task that normalizes the two fixed agent model fields, so every copy and update command requires `--trust`.
...
After `.copier-answers.yml` records a v1-or-newer release and `.project-agent-workflow/` exists, use ordinary `copier update --trust` for later template versions.
