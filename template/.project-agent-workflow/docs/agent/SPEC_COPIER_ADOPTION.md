...
The template supplies those two fields as defaults only when they are absent. A value the project already declares stays exactly as declared, even when it equals an older seed, and instructions and every unrelated field are preserved.
...
Template development must preserve project-owned product code, repository policy, configuration, plan history, and repository-specific validation behavior across every supported copy and update path.

An update is not acceptable merely because Copier exits successfully.

Supported update fixtures must finish without unresolved Git conflicts, `*.rej` files, inline conflict markers, or unclassified tracked-file deletion, and repository-specific validation must remain runnable.

Copier-managed files may change as part of the intended update, and the two agent model fields may appear where the project had not declared them.
...
The final validator rejects changes to existing project-owned or unclassified paths except the insertion of an absent `model` or `model_reasoning_effort` default and the exact recognized sequential-worker transition. Replacing a model field the project already declared is rejected.
...
1. Run `copier copy --trust` with the intended answers.
...
2. Run `.project-agent-workflow/scripts/update-from-copier.sh` without force-overwriting local conflicts.
3. Confirm that project-owned files are unchanged except for a newly inserted `model` or `model_reasoning_effort` default in seeded `.codex/agents/*.toml` files.
...
This template always runs a post-render task that fills the two agent model fields when they are absent, so every copy and update command requires `--trust`.
