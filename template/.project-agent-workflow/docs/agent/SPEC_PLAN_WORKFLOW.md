...
- A review-budget-exhausted `descope_pending` run with reason `parent_remediation_budget_exhausted`, no open writable attempt, and exactly two formal reviews may receive one owner-authorized same-plan continuation. Never reopen or modify the stopped ledger. Create a fresh ledger with `plan-execution-state.py continue`, bind the complete predecessor-ledger digest and event-chain leaf, exact unchanged plan path and digest, source HEAD, primary invariant, implementation mode, and a mode-0600 authorization record.
