...
- Rebind every pre-existing live plan that names an archived plan's former active path in `context_files` inside the restructuring transaction that performs the archival, and reject that transaction when such an entry would survive it. A live plan is a plan listed in `docs/plan/plan.md` or a file under `docs/plan/backlog/`. This removes the exemption above on the transaction path without weakening it anywhere else.
...
- Leave a lifecycle-protected contract successor on the exemption above instead of rewriting it. Rewriting it would break the protected manifest comparison, and rejecting it would deadlock the transaction, because a single-source specification carries no rebinding field and a rebinding cannot target a plan already in the backlog. Closing that remaining case needs the rebinding channel those routes lack.
