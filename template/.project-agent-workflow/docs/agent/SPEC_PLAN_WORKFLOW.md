...
Conversation may continue in any session while a task is being implemented in its bound worktree.
Stop reminders about repository-wide work grant no authority to continue or finalize another session's task.

- To author a plan in a separate session, run `python3 .project-agent-workflow/scripts/manage-plan-worktrees.py prepare --direct-task <unique-authoring-id>` from the source checkout, then perform writes in the returned worktree. Publish that authoring task before implementation starts.
- To begin implementation from another session, run `python3 .project-agent-workflow/scripts/manage-plan-worktrees.py prepare docs/plan/active/NNN-slug.md` for the published active plan and work in the returned checkout.
- An existing task checkout can be resumed, but only after its previous session has stopped writing. Preparation does not prove the old session is idle; it is not permission for two sessions to write the same task concurrently. Keep an explicitly assigned owner id consistent when resuming.
- Different direct authoring tasks may coexist with a plan worktree. Shared identifier allocation still applies; two numbered plans may run together only through the existing execution-group admission.
- Publish tasks serially. A dirty or drifted source checkout still refuses publication; a new session does not waive that refusal or reset execution and review budgets.
...
- Supported main-agent Stop hooks provide read-only reminders through `.project-agent-workflow/hooks/stop_review_gate.py`. One adapter serves Codex `Stop` and Copilot `agentStop` configured in `.github/hooks/plan-lifecycle.json`. Every call returns one empty decision with exit code zero, including missing gates, failed checks, unavailable diagnostics and retained worktrees. Each diagnostic subprocess has a five-second timeout. Stop never forces continuation, infers completion from message text, binds a session to another task, or reads or writes repetition counters. `stop_hook_active` skips repeated diagnostics. The adapter is never attached to `subagentStop`. Ending a conversation turn is not a completion claim; write guards, pre-commit, CI and publication remain the enforcement boundaries.
