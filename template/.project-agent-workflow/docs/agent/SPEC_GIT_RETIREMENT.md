...
A worktree prepared by `.project-agent-workflow/scripts/manage-plan-worktrees.py` for the current task is retired by that manager, not by this workflow.
...
`.project-agent-workflow/scripts/retire-merged-worktrees.py scan` is read-only and `.project-agent-workflow/scripts/retire-merged-worktrees.py apply-local` is an explicit local effect.
...
`.project-agent-workflow/scripts/manage-plan-worktrees.py` unlinks the ownership record it owns, and needs the repository that record names to do it.
...
`.project-agent-workflow/scripts/retire-stale-worktree-records.py scan` is read-only and `.project-agent-workflow/scripts/retire-stale-worktree-records.py apply-local` is an explicit local effect.
