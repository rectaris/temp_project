...
def policy_errors() -> list[str]:
    errors: list[str] = []
    index = ROOT / "docs/agent/spec-index.yaml"
    if not index.is_file():
        return errors
    text = index.read_text(encoding="utf-8")
    if not text.startswith("version: 1\n"):
        errors.append("docs/agent/spec-index.yaml must start with version: 1")
    for key in ("name", "slug"):
        match = re.search(rf"^  {key}:\s*(.+)$", text, re.MULTILINE)
        if not match:
            errors.append(f"docs/agent/spec-index.yaml missing project.{key}")
            continue
        try:
            value = json.loads(match.group(1))
        except json.JSONDecodeError:
            errors.append(f"docs/agent/spec-index.yaml project.{key} must be a JSON-compatible quoted scalar")
            continue
        if not isinstance(value, str) or not value:
            errors.append(f"docs/agent/spec-index.yaml project.{key} must be a non-empty string")
    referenced = sorted(set(re.findall(r"docs/agent/[A-Za-z0-9_.-]+", text)))
