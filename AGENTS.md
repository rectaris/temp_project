...
- Defer a replan successor to `docs/plan/backlog/` with `status: backlog` instead of restructuring it again when it is merely deprioritized. Keep its contract-recorded path, acceptance, and inherited digests unchanged, drop the stopped-lifecycle fields, and keep it resolvable in exactly one lifecycle location.
