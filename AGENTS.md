...
- Use `scripts/run-sandboxed-plan-worker.py` for writable sequential-plan implementation; keep `.codex/agents/sequential_plan_worker.toml` read-only and apply only parent-admitted candidate patches. Every writable delegated plan must declare one nonblank `primary_invariant` and exact file paths in `write_scope`; directory prefixes, protected inputs, and validation-authority paths fail before worker start. Require a canonical network `remote.origin.url`; the runner supplies a freshly derived, repository- and plan-bound contract as read-only input for each attempt.
