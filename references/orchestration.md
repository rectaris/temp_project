...
- Before a plan uses `npm run` validation, the parent runs `run-sandboxed-plan-worker.py prepare-dependencies --output-dir <new-external-directory>` against a clean repository with its project-approved `node_modules`, then passes the emitted manifest to `validate --dependency-snapshot <manifest>`. Preparation creates that new directory as runner-owned mode `0700`, requires the installed package records to match `package-lock.json`, copies the dependency tree outside the repository, and records its complete digest. Validation rechecks the package, lockfile, source HEAD, and tree digests, copies the verified tree into parent-private temporary storage, and mounts only that private copy read-only into each network-disabled fresh clone. This proves which reviewed local tree was supplied; it does not independently establish registry provenance for the original installation.
