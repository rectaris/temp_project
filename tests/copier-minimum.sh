...
UV_CACHE_DIR="$root/.uv-cache" UV_TOOL_DIR="$tmp/uv-tools" uvx --python "$python_path" --from "copier==$copier_version" copier copy -q -f --vcs-ref "$source_ref" --data-file "$root/tests/fixtures/docs.answers.yml" "$root" "$tmp/generated"
