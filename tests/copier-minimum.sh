...
source_ref=${COPIER_MINIMUM_REF:-HEAD}
python_path=$(command -v python3)
UV_CACHE_DIR="$root/.uv-cache" UV_TOOL_DIR="$tmp/uv-tools" uvx --python "$python_path" --from "copier==$copier_version" copier copy -q -f --trust --vcs-ref "$source_ref" --data-file "$root/tests/fixtures/docs.answers.yml" "$root" "$tmp/generated"
