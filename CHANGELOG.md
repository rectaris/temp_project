...
- 正規の停止記録がないまま確定してしまった過去の再計画契約を、名指しで解消できるようにしました。
  アーカイブ済みの再計画契約は書き換えられません。
  そのため、記録された停止前の計画本文に `status: replan_required` が付いていない契約が一件でも残っていると、その配布先は計画検査を二度と通せなくなります。
  契約を作る時点では当時から停止が必ず求められていたので、これは版の違いではなく個別の異常です。
  版をひとまとめに免除すると、すべての配布先へ恒久的な抜け道を開くことになります。
  そこで `docs/plan/replanned/baselines/pre-boundary-contract-sources-v1.json` に、対象の契約を一件ずつ書き出す方式にしました。
  受け入れるのは、契約とアーカイブの双方が、HEAD の祖先である境界コミットの時点ですでに同じ内容で確定している場合だけです。
  この記録自体も一度しか書き込めません。
  境界より後に生じた食い違いは受け入れられず、新しい再計画が停止を経ずに契約を作ることもできません。

## 2026-09-13 v1.4.8

- 到達できなくなった所有記録を、退避して取り除けるようにしました。
  タスクワークツリーの管理コマンドは自分の所有記録を消すために、その記録が指すリポジトリを必要とします。
  そのため一時的なリポジトリが消えると記録だけが残り、共有ディレクトリがガードの読み取り上限に達して、同じ利用者のすべてのリポジトリが止まります。
  `retire-stale-worktree-records.py` の `scan` が判断材料を書き出し、`apply-local` がその内容をもう一度確かめてから記録を隣の `retired` ディレクトリへ移します。
  パスが存在しないことはリポジトリが消えた根拠にはなりますが、証明にはなりません。検査は一つのマウント名前空間しか見えず、デバイス番号は再起動で再利用されるためです。
  そこで削除ではなく移動にしました。判断を誤っても、失うのは記録ではなく操作者の移動一回です。
  退避した記録が生きたリポジトリに一致した場合は判断が誤っていた合図なので、未完了タスクの報告は退避先も読んで印を付け、記録を作るすべての経路が退避済みのタスクを拒否し、停止ゲートは復元の手順を案内します。

- テストが作った所有記録を、テスト自身が片付けるようにしました。
  所有記録のディレクトリは同じ利用者のすべての実行で共有され、記録が 512 件を超えるとガードが読み取りを拒否します。
  タスクワークツリーを用意するテストが記録を残していたため、検証を繰り返すほど上限に近づき、変更内容とは無関係な理由で失敗していました。
  各テストは用意を始める前に記録・ジャーナル・ロックの所有を宣言するので、途中で失敗した場合も片付きます。
  すでに存在するパスの所有は拒否するため、他の実行が持つ記録を消すことはありません。
  終了時に処理する保留集合が中断の場合を補います。unittest の後片付けはそこまで届きません。

- 追跡対象ファイルの文字面の検査を、手元の検証でも行うようにしました。
  v1.4.7 のリリースは、ファイル末尾の空行で CI が止まりました。
  その規則は CI にしかなく、コミットの範囲を読むため、どの範囲を取るかによって結論が変わります。
  そこで追跡対象のファイル全体について Git 自身に同じ判定を求めます。空のツリーを索引および作業ツリーと比較します。
  Git の終了状態が判定を決めるため、このコマンドが解釈できない出力であっても拒否は成立し、Git 自身のバイナリ判定・改行変換・パス属性がそのまま効きます。
  規則を一つだけ加えました。ファイル末尾の改行の欠落です。Git はこれを報告しますが、拒否はしません。
  すでに規則に反していた 5 つのファイルを直したので、除外一覧は要りません。

## 2026-09-13 v1.4.7

- 配布先で見つかったテンプレートの改善案を、報告から開発方針まで根拠を追える形で運べるようにしました。
  配布先は `report-template-feedback` で改善案と根拠を記録し、テンプレート側は `collect-template-feedback` でそれを取り込んで、出所をたどれる要求候補にまとめます。
  採否は候補の版に結び付けて記録し、一つの要求には生きた判断を一つだけ許します。後から判断を変える場合は、どの判断を置き換えるのかを明示する必要があります。
  `development-direction` は採用済みの判断から `docs/development-direction.md` の生成区間だけを書き直し、手書きの前後の文章はそのまま残します。
  取り込んだ報告の文章は文書の中でも常に根拠として扱い、見出しや区間境界を作れない一行の値として書き出します。規則を満たせない要求は、内容を文書へ入れずに理由付きで「未描画」と並べます。

- 補助指示の版を選び、採用の可否を実行記録から判断できるようにしました。
  `harness-profile` で補助指示の版を明示的に選択し、`compare-harness-runs` で同じ課題の実行記録を突き合わせて、モデルの変更による差と指示の変更による差を分けて読み取れます。
  比較は手元の記録だけで完結し、以前の版へ戻す判断も同じ記録から行えます。

- 計画索引と完了記録の判定を、表示経路と検査経路で一本化しました。
  計画一覧の表示と実行前検査が同じ規則で `docs/plan/plan.md` を読み、完了記録の索引が指す保存先も共有の検査を通します。
  起動時に参照する完了記録は、マニフェストの版を問わず同じ規則で読めるため、古い記録を持つ配布先でも参照が途切れません。

- 生成テストが、未コミットの変更を含む現在の入力ファイルから生成物を描画するようにしました。
  検査対象と手元の編集内容が一致するため、コミット前に生成先の姿を確かめられます。

- プランの実装中でも会話を終えられるようにしました。
  作業が進行中であることを理由に応答を止めず、別のセッションから続きを始められます。

## 2026-09-10 v1.4.6

- Copier 更新を適用できるかどうかの判断を、文章での推論ではなく、コミット済みデータで一意に決まる結果へ移しました。
  検証の各結末に理由コードを与え、責任の所在と次に取るべき行動を表から引けるようにし、理由コードの網羅を検査で固定します。
  配布先プロジェクトを `docs/downstream-baselines.yaml` に記録し、リリース前に全件をまとめて検証できるようにしました。
  各プロジェクトが実際に持つ検証ゲートを記録し、プロジェクト自身の検証コマンドを一度も実行していない検証結果には、その旨を結末に明記します。

- 計画索引の判定を一本化し、まだ採用していない文書をその名前で呼ぶようにしました。
  `docs/plan/active/` の有無による短絡をやめ、`docs/plan/plan.md` が索引として採用されているかどうかだけで判定します。
  採用前の文書は「壊れている」ではなく「未採用」として報告するため、生成先で索引の作成と削除が繰り返されなくなります。

- マニフェストに項目が加わる前に書かれた確認済みアーカイブを、読めるようにしました。
  `status`、`review_class`、`human_design_required`、`human_approval_status`、`acceptance` は、その記録が実際に持つ場合にだけ判定します。
  `status` を持たない記録は、置かれている場所から `checked` として読み、先行計画の検査も同じ経路を通します。
  項目を書いたうえで値を空にした記録は、従来どおり欠陥として拒みます。
