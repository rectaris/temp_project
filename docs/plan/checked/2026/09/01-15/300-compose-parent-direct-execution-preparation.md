...
- [x] Add fresh parent-direct setup tests and injected failures at each existing initialization boundary.
- [x] Extract minimal shared preflight helpers and compose existing registry/state initialization functions.
- [x] Test that any initially occupied or unsafe later destination fails before the first record is created; test races separately as retained partial failure. Verify resulting records with the three named readers and explicit identity comparisons.
- [x] Test path identity, nonoverlap, concurrent creation, retained partial state and refusal of occupied or stopped state.
- [x] Update aligned orchestration and existing sequential skill guidance for fresh parent-direct setup only.
- [x] Run focused tests, independent review and one authoritative suite for the reviewed patch.
...
- 2026-09-09 epoch 1 remediation: Replaced the reserved-pathname fix with a no-replace publication. `atomic_write` gained a `create_new` parameter; the first execution-state publication installs the fully written and flushed temporary file under its final name with `os.link`, so the name appears only with complete content, never replaces an occupied name, and leaves no placeholder when an attempt fails. `reserve_new_execution_state` was removed and every existing caller keeps the previous replacing rename unchanged. Candidate commit `eb3df0e`.
- 2026-09-09 epoch 1 review: A fresh independent reviewer verified the publication mechanism, the preflight ordering, the authority-equivalence normalization, the byte-identical mirror, the policy wording and all five integration gates, and reported no High and no Medium findings. It reported one Low finding: the failed-publication test replaced `atomic_write` itself, so its cleanup assertion held for the wrong reason. Corrected in `8b00aa5` by failing the publication link instead; an adversarial preflight confirmed the test now fails when the cleanup is removed.
- 2026-09-09 epoch 1 confirmation review: The second and final review of the epoch verified the corrected test, its scoping and its mutation sensitivity, confirmed no production code changed since the clearing review, and reported no High findings. Its single Medium finding, that the candidate range touches an eighth path outside `write_scope`, is a misapplied criterion supplied in the review request rather than a defect: the eighth path is this plan's own Validation Notes, which are a parent-owned lifecycle record. No plan in this repository lists its own file in `write_scope`, including checked plan 276, and the admission rule treats plan-lifecycle records as a category distinct from `write_scope`. The disposition is recorded here rather than accepted as a stop.
- 2026-09-09 authoritative validation: `./scripts/lint-project-workflow.sh` exit 0 and `REQUIRE_COPIER=1 ./tests/smoke.sh` exit 0, run once for the reviewed candidate. Focused validation at the same commit: `python3 tests/test-plan-execution-state.py` 164 tests OK, `python3 scripts/check-copier-template.py` passed, `python3 scripts/check-root-agent-policy.py` exit 0, `git diff --check` clean, and `python3 tests/test-sandboxed-plan-worker.py` 146 tests OK.
- 2026-09-09 completion: All four completion conditions are met. One `prepare-parent-direct` call composes the three existing fresh-start constructors, derives its digests from the exact committed active plan, refuses every unsafe destination before writing, creates the ledger last and reports readiness only after the three named readers accept the records; a partial failure names what is retained and never deletes, replaces or retries; the resulting state is structurally identical to the three-call sequence, so no implementation, review, continuation or publication authority is added; and the aligned orchestration policy and sequential skill guidance describe fresh parent-direct setup only. Candidate commits `efe973b`, `fb3d749`, `974505a`, `eb3df0e`, `8b00aa5`. Three independent reviews across two execution epochs, with one owner continuation authorization.
