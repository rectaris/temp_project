...
integration_gates:
  - The owner approved this class C design and authorized this one reconstruction; do not widen the boundary further without a new owner decision.
  - This reconstruction narrows write_scope to the files the relocation actually changes; the plan workflow and security specifications stay read-only context.
  - Change tests/smoke.sh only where it asserts that the generated entrypoint restates the relocated guardian rule; keep every unrelated assertion, including the validation-witness map assertions, byte-identical.
...
checked_summary_ja: 起動時の AGENTS.md から重複する詳細手順を規範文書へ移し、作業別の直接参照と安全条件の検査を残す。移設に伴い陳腐化する正式検証の判定も同時に更新する。
