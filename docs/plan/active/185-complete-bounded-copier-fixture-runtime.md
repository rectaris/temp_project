...
status: deferred
completion_deferred_reason: Plan 191 must be checked and its exact checked archive path must replace the active predecessor before implementation.
...
validation_witness_map:
  - {"acceptance_sha256":"sha256:251de7e9d22d4d2657f9b3890114005a890bceab1a44b54dda2f63cf690d96d1","stage":"focused","witness":"python3 scripts/project_workflow/copier_fixture.py --check tests/copier-update.sh"}
predecessor_plans:
  - docs/plan/active/191-freeze-bounded-copier-fixture-validator.md
...
integration_gates:
  - Plan 191 must be checked and its exact checked archive path must replace this active predecessor before implementation
  - in the same parent-owned activation update, add scripts/project_workflow/copier_fixture.py emitted by checked Plan 191 as exact read-only context
...
- Validate the runtime only with the checked bounded_copier_fixture_validator from Plan 191 and do not use the dirty broad checker as acceptance authority.
