...
predecessor_plans:
  - docs/plan/active/102-select-runnable-active-plan.md
integration_gates:
  - Plan 102 must be checked and its exact checked archive path must replace this active predecessor before implementation
...
- [ ] Extend the root and generated restructuring schema, verifier, and atomic rollback tests.
- [ ] Enforce disjoint preservation_scope and write_scope effects at every candidate and parent-owned lifecycle boundary.
- [ ] Add positive, tampering, overlap, dropped-path, and legacy schema tests.
- [ ] Complete focused validation and independent review with zero unresolved High or Medium findings.
- [ ] Run the authoritative suite once, archive, commit, and activate Plan 189 with the exact checked predecessor path.
