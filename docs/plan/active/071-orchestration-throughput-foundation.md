...
acceptance:
  ...
  - Default an absent risk or ambiguity field to ordinary, but reject an explicitly present blank, whitespace-only, list-valued, or unknown classification.
  ...
  - Require the fixture checker to prove Spark, Terra, both high-input refusals, a successful nondefault model and reasoning override, and preferred and fallback writable-Sol refusals; broad repository scope alone must not satisfy the delegation value gate.
  ...
  - Include every changed generated orchestration policy, plan parser, runner, and Skill in the simulated Copier update source and assert their routing semantics after update.
...
- Rejected routing-slice correction candidate `/tmp/orchestration-plan-071-routing-correction1-otHMpy/manifest.json`, source HEAD `3dee2865cabb4b1b00e79f867c8e637f36d731b6`. One GPT-5.6-Terra medium attempt generated the candidate without fallback.
- Review-clone validation passed 39 runner tests, runner self-test, root policy, Copier static checks, root/template runner parity, Python compilation, and diff checks. The correction candidate was not applied because fixed scenarios omitted successful explicit override and writable-Sol cases, explicit blank plan classifications were treated as omitted, and simulated Copier update sources omitted the changed generated orchestration artifacts.
