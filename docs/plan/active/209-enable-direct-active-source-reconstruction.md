...
status: deferred
completion_deferred_reason: Plan 208 must be checked and its exact checked archive path must replace the active predecessor before implementation.
...
predecessor_plans:
  - docs/plan/active/208-bind-journal-replacement-identity.md
integration_gates:
  - begin only from the exact checked Plan 208 archive and a clean worktree
  ...
  - Plan 210 remains deferred until this plan is checked and its exact checked archive replaces the active predecessor
...
- [ ] Run the authoritative suite once, archive, commit, and activate Plan 210 with this plan's exact checked archive as predecessor.
