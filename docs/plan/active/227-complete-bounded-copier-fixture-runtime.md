...
- [x] Re-admit only the preserved tests/copier-update.sh candidate against checked Plans 141, 180, and 182.
- [x] Confirm the synthetic v1.4.5 tag has its own ordered commit and the fixture selects v1.4.4 then v1.4.5 through Copier without invoking the snapshot script directly.
- [x] Confirm the downstream project commits the v1.4.4 pre-schema active plan, replanned source archive, and replan contract before the update begins and that the consumed record captures that active plan.
- [x] Confirm the ready emission, two bounded polling loops, three release paths, update-child TERM/grace/KILL/final reap, PID clear, pending then consumed assertions, positive guardian PID, and guardian cleanup.
- [x] Complete fresh independent review and focused validation through the checked bounded validator with zero unresolved High or Medium findings.
...
- Task 2 groundwork, read-only. The v1.4.5 transition is wired as a Copier migration, not as a step of the update wrapper. `copier.yml` lines 91 to 105 declare one `before` and one `after` migration for version v1.4.5, each running `scripts/snapshot-validation-witness-provenance.py` from the template source path, and `scripts/check-copier-template.py` enforces exactly that pair. The generated wrapper `template/.project-agent-workflow/scripts/run-copier-update.sh` names only `validate-copier-update.py`, so the fixture reaches the guardian by running the wrapper and letting Copier run the migration, which is also why the checker forbids naming the snapshot script directly.
- Task 4 groundwork, read-only. The test failpoints in the snapshot script only exit the process; none of them pause it. The before stage starts a detached guardian, publishes the `pending` attempt state, and returns, so the fixture cannot observe that state by racing an unpaused update. The remaining design question this plan must answer before writing the runtime is how the fixture holds the update child between the before and after migrations so the `pending` state, the guardian identifier, and the release paths are observed, and that answer must not name the snapshot script directly.

- Task 4 design answer, implemented. The fixture holds the update child with a third, fixture-owned v1.4.5 `before` migration written into the disposable `$update_source/copier.yml` directly after the snapshot before migration. Its command is a fixture-written hold script under `$tmp` that emits the ready path and then bounded-polls for the release path, so the fixture observes the published `pending` state, the guardian identifier, and every release path without naming the snapshot script.
- The insertion anchor is selected by version-scoped entry boundaries rather than by the `when` line, because the plain `- "before"` and `when` pair also matches a v1.0.0 migration. A read-only YAML parse confirmed the resulting v1.4.5 order is snapshot before, hold, snapshot after.
- Both synthetic boundaries use `git tag -f`, because `fixture_clone` copies the released `v1.4.4` tag of this repository into the disposable clone and a plain `git tag v1.4.4` aborts the fixture. An isolated clone confirmed the plain tag fails and the forced tag repoints to the synthetic boundary.
- The policy marker removal and restoration round-trip is byte-identical, so the synthetic v1.4.4 boundary is the committed template without the installed marker and the synthetic v1.4.5 boundary restores it exactly.
- `prepare_lane` dispatches the pre-v1 update to the written literal `"$tmp/earliest-supported"`, because the checker treats every function-assigned name as unsettled and the destination-aware rule then cannot separate that lane from the update child.
- Cleanup carries the Plan 183 bound. It writes the release path, waits 30 seconds for the update child, then TERM, a 5-second grace, KILL, a final reap, and the PID clear, and only then recovers the guardian identifier from the published attempt state and stops it. An isolated run with a TERM-ignoring child completed in 35 seconds, stopped a recovered detached guardian, and removed the temporary root.
- The hold script also exits when the temporary root disappears, so a cleanup that removes `$tmp` retires the held migration within about one second instead of leaving it to its 600-second bound.
- The `kill -0` wait bound is kept. A completion-marker child is structurally rejected by the acceptance authority: a brace group makes both inner commands asynchronous, and `sh -c` wrapping makes the update destination unreadable and rejects all 19 other update paths.
- Validation: `sh -n tests/copier-update.sh`, `python3 scripts/project_workflow/copier_fixture_validator.py --check tests/copier-update.sh`, and `git diff --check` all pass. The fixture itself was not executed; Plan 179 keeps that authority.
- Independent review ran three rounds with one read-only helper that held no write scope and whose output stayed advisory until this session accepted it. Round 1 raised four Medium findings, round 2 raised one Medium finding, and all five were repaired and re-validated. Round 3 raised no High or Medium finding inside this change.
- Round 3 also reported that `cleanup` captures `$?` for HUP, INT, and TERM, so a signalled run can exit 0. That defect is in the committed handler this change does not touch, is outside this plan's invariant and acceptance, and is recorded here as follow-up work rather than repaired here.
