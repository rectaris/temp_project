...
- [ ] Add one fail-closed helper that revalidates the exact manifest path and compares the parsed digest-valid value to the startup snapshot.
- [ ] Invoke it immediately before each deletion command without releasing the existing target locks.
- [ ] Add deterministic manifest replacement or mutation races before worktree removal and before branch deletion, and assert the pending command is not invoked.
- [ ] Keep root and generated executables byte-identical and mode `0755`.
- [ ] Complete parent diff review, independent security review, focused validation, and one authoritative behavior validation before acceptance.
