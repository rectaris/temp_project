...
- To author a plan in a separate session, run `python3 scripts/manage-plan-worktrees.py prepare --direct-task <unique-authoring-id>` from the source checkout, then perform writes in the returned worktree. Publish that authoring task before implementation starts.
- To begin implementation from another session, run `python3 scripts/manage-plan-worktrees.py prepare docs/plan/active/NNN-slug.md` for the published active plan and work in the returned checkout.
